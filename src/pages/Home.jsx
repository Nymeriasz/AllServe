...
import { FaWhatsapp, FaInstagram, FaStore, FaGlassCheers, FaBuilding, FaMapMarkerAlt } from 'react-icons/fa';
...
const BartenderHome = () => {
  ...
  useEffect(() => {
    const fetchEmpresas = async () => {
      try {
        ...
        const q = query(usersRef, where("role", "!=", "bartender")); 
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (data.nome) {
            listaEmpresas.push({
              id: doc.id,
              nome: data.nome,
              tipo: data.tipo || "Cliente AllServe",
              local: data.local || "Localização não inf.",
  ...
  const getStatusColor = (status) => {
    if (status === 'Contratando') return 'green';
    if (status === 'Evento Próximo') return 'purple';
    if (status === 'Ausente') return 'red';
    return 'blue';
  };

  const getIcon = (tipo) => {
    if (tipo && tipo.toLowerCase().includes('restaurante')) return FaStore;
    if (tipo && tipo.toLowerCase().includes('hotel')) return FaBuilding;
    return FaGlassCheers;
  };

  if (loading) {
    return <Center h="50vh"><Spinner size="xl" color="#c49b3f" /></Center>;
  }
      ...
      <Container maxW="container.xl">
        <VStack spacing={8} align="stretch">
          <Box textAlign="center" mb={4}>
            <Heading size="xl" color="gray.700">Bem-vindo ao <Text as="span" color="#c49b3f">AllServe</Text></Heading>
            <Text fontSize="lg" color="gray.500">Veja quem está contratando e movimentando a plataforma hoje.</Text>
          </Box>

          <Heading size="md" color="gray.600">Empresas e Clientes Parceiros</Heading>
            ...
            <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={6}>
              {empresas.map((empresa) => (
                <Card 
                  ...
                  bg="white"
                  shadow="md" 
                  _hover={{ shadow: 'xl', transform: 'translateY(-2px)', borderColor: '#c49b3f' }} 
                  transition="all 0.2s"
                  cursor="pointer"
                  overflow="hidden"
                  border="1px solid"
                  ...
                  <Flex w="100%">
                    ...
                    >
                      {empresa.fotoURL ? (
                          ...
                          <Center w="100%" h="100%">
                             <Icon as={getIcon(empresa.tipo)} w={12} h={12} color="gray.300" />
                          </Center>
                      )}
                    </Box>
                    ...
                    <CardBody py={4} px={5} display="flex" flexDirection="column" justifyContent="center">
                      <VStack align="start" spacing={2}>
                        <Flex justify="space-between" w="100%" align="center">
                          <Heading size="md" color="#292728" noOfLines={1}>{empresa.nome}</Heading>
                        </Flex>
                        
                        <HStack>
                            <Badge colorScheme={getStatusColor(empresa.status)}>{empresa.status}</Badge>
                            <Text fontSize="sm" fontWeight="bold" color="gray.500" noOfLines={1}>{empresa.tipo}</Text>
                        </HStack>

                        <HStack color="gray.500" fontSize="sm" pt={2}>
                          <Icon as={FaMapMarkerAlt} color="#c49b3f" />
                          <Text noOfLines={1}>{empresa.local}</Text>
                        </HStack>
                      </VStack>
                    </CardBody>
                  </Flex>
...
const InitialServiceCard = ({ service, onClick }) => (
    ...
    >
      <Image src={service.imgSrc} alt={`Ilustração ${service.name}`} maxH="180px" width="100%" objectFit="cover" />
        ...
        >
            ...
            <Box px={3} py={3}> 
                ...
                <Flex justify="space-between" align="center" mt={2}>
                  ...
                  <HStack spacing={2}>
                      {whatsappLink && (
                          <Link href={`https://wa.me/${whatsappLink}`} isExternal bg="#25d366" color="white" h="24px" w="24px" borderRadius="full" display="flex" alignItems="center" justifyContent="center" _hover={{ opacity: 0.8 }} onClick={(e) => e.stopPropagation()}>
                              <Icon as={FaWhatsapp} fontSize="14px" />
                          </Link>
                      )}
                      {instagramLink && (
                          <Link href={`https://instagram.com/${instagramLink}`} isExternal bgGradient="linear(to-r, #f09433, #e6683c, #dc2743, #bc1888)" color="white" h="24px" w="24px" borderRadius="full" display="flex" alignItems="center" justifyContent="center" _hover={{ opacity: 0.8 }} onClick={(e) => e.stopPropagation()}>
    ...
    <Box>
      ...
      <Box p={8} textAlign="center" py={{ base: 12, md: 20 }} bg="white" id='sobre'>
          <Container maxW="container.xl">
              ...
              <Button as={RouterLink} to="/sobre" variant="secundario" mt={12} size="lg">
